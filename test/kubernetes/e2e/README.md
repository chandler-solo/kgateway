...
2. The `make unit` command will not run e2e tests; `make e2e-test` does. To run a specific e2e test, you can use `go test` directly with the `E2E=true` environment variable. E2E tests check this variable at runtime and skip themselves if it's not set to a truthy value (true or 1).

To run a specific test suite directly (everything that starts with `TestKgateway`):
```shell
E2E=true go test -v -timeout 600s ./test/kubernetes/e2e/tests -run ^TestKgateway
```
...
```shell
E2E=true go test -v -timeout 600s ./test/kubernetes/e2e/tests -run ^TestKgateway$$/^BasicRouting$$
```
