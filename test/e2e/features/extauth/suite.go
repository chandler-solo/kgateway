...
func NewTestingSuite(ctx context.Context, testInst *e2e.TestInstallation) suite.TestingSuite {
	...
	testCases := map[string]*base.TestCase{
		...
		"TestExtAuthEnvoyAcceptsAuthCluster": {
			Manifests: []string{securedGatewayPolicyManifest},
		},
...
// TestExtAuthEnvoyAcceptsAuthCluster asserts that the ext_authz cluster the policy
// references actually lands in Envoy's CDS. The policy's filter is only useful if the
// cluster backing it survives translation and is accepted by the proxy.
