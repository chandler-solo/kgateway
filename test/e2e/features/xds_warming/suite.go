...
func (s *testingSuite) AfterTest(suiteName, testName string) {
	if !testutils.ShouldSkipCleanup(s.T()) {
		err := s.TestInstallation.Actions.Kubectl().DeleteFileSafe(s.Ctx, backendNewManifest)
...
func (s *testingSuite) TestRouteUpdateWaitsForNewEDSBeforeBreakingOldTraffic() {
	...
	s.assertGatewayEventuallyServes(oldBody)
	...
	s.assertActiveClusterPresence(oldClusterName, true)
	s.assertGatewayServesConsistently(oldBody, 10*time.Second, 500*time.Millisecond)
	...
	s.assertGatewayEventuallyServes(newBody)
	s.assertGatewayServesConsistently(newBody, 5*time.Second, time.Second)
}
...
func (s *testingSuite) assertGatewayEventuallyServes(bodySubstring string) {
	s.TestInstallation.AssertionsT(s.T()).AssertEventualCurlResponse(
		s.Ctx,
		testdefaults.CurlPodExecOpt,
		s.gatewayCurlOptions(),
...
func (s *testingSuite) assertGatewayServesConsistently(bodySubstring string, window, poll time.Duration) {
	s.assertGatewayServesOnce(bodySubstring)

	s.TestInstallation.AssertionsT(s.T()).Gomega.Consistently(func(g gomega.Gomega) {
		statusCode, body, err := s.gatewayResponse()
...
func (s *testingSuite) assertGatewayServesOnce(bodySubstring string) {
	statusCode, body, err := s.gatewayResponse()
	s.Require().NoError(err)
...
func (s *testingSuite) gatewayResponse() (int, string, error) {
...
func (s *testingSuite) gatewayCurlOptions() []curl.Option {
	return []curl.Option{
