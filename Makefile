...
# To run only e2e tests, we restrict to ./test/kubernetes/e2e/tests and set
# E2E=true environment variable. E2E tests check this variable at runtime and
# skip themselves if it's not set to a truthy value (true or 1). This approach
# allows unit tests to run faster since e2e tests are skipped, and prevents
# errors when running unit tests without a kind cluster.
...
run-load-tests: ## Run KGateway load testing suite (requires existing cluster and installation)
	E2E=true SKIP_INSTALL=true CLUSTER_NAME=$(CLUSTER_NAME) INSTALL_NAMESPACE=$(INSTALL_NAMESPACE) \
