...
type GracefulShutdownSpec struct {
	...
	// filter starts returning 503; other replicas are unaffected. This only gives
	// the external LB per-pod health when the LB target group targets Envoy pod
	// IPs directly. If the target group targets Kubernetes nodes, or any path
	// where kube-proxy load-balances the health check before it reaches Envoy,
	// the LB health result applies to that node or load-balanced path rather than
	// to an individual Envoy pod, and a probe may be answered by a different
	// ready pod. If an external LB target probe reaches the terminating pod's IP
	// at a port whose Envoy listener has the HTTP health check filter installed
	// in its filter chain, that probe will start receiving 503s during the sleep
	// window and the LB will drop that pod target from rotation. Without an HTTP
	// health check filter on the data plane, /healthcheck/fail has no externally
	// visible effect and only the sleep matters.
	//
	// The external LB does not need access to Envoy's admin port. It reaches the
	// health check through a data-plane Gateway listener port; kgateway adds
	// Gateway listener ports to the generated proxy Service so cloud LB
	// controllers can derive their targets from that Service. A separate Service
	// port is only needed if you put the health check on a dedicated Gateway
	// listener/port instead of an existing traffic listener.
	//
	...
	//
	// For AWS Application Load Balancers and Network Load Balancers that should
	// make per-pod health decisions, use target groups that register Envoy pod
	// IPs (target type ip). For ALBs, configure the target group's HTTP or HTTPS
	// health check to use a health check port that reaches the Gateway listener
	// with the Envoy filter and set HealthCheckPath to the filter's path. For
	// NLBs, configure the target group's HealthCheckProtocol as HTTP or HTTPS and
	// set HealthCheckPath to that same path if the NLB should observe the Envoy
	// drain signal; the default TCP health check only verifies that the port
	// accepts connections, so it will not notice /healthcheck/fail. If the AWS
	// target group uses instance target type, target health is node-level rather
	// than pod-level.
	//
