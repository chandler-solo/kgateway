...
func allEnvVarsSet() map[string]string {
	return map[string]string{
		...
		"KGW_DISABLE_STATS_ON_PROXY":                    "true",
...
// defaultSettings returns the complete Settings produced when no env vars are
// set. It must match every field's struct-tag default, since it is the baseline
// that precedence tests compare the full Settings struct against.
func defaultSettings() *Settings {
	return &Settings{
		...
		WorkloadEntriesExclusionLabels:        "",
