...
const (
	...
	// ValidatorDaemon adds a bounded concurrency pool on top of ValidatorCache.
	ValidatorDaemon ValidatorMode = "DAEMON"
)
...
func (v *ValidatorMode) Decode(value string) error {
	...
	switch mode {
	case ValidatorBinary, ValidatorCache, ValidatorDaemon:
		*v = mode
		return nil
	default:
...
type Settings struct {
	...
	// - "BINARY": fork envoy --mode validate per call (default; preserves prior behavior).
	// - "CACHE": wrap BINARY with an LRU result cache keyed on bootstrap content hash.
	// - "DAEMON": additionally bound concurrent envoy invocations with a worker pool.
	ValidatorMode ValidatorMode `split_words:"true" default:"BINARY"`

	// ValidatorCacheSize is the LRU capacity used by CACHE and DAEMON validator modes.
	...
	// ValidatorPoolSize is the worker count for DAEMON validator mode. Ignored otherwise.
