...
`XdsEnvoyWarming.tla` is a focused startup and make-before-break model for Envoy active state. It separates ACKed resources, initialized clusters, active routes/listeners, and missing/empty/ready CLAs. Empty CLAs permit cold initialization, while the warm route-flip policy requires ready endpoints. `XdsEnvoyWarmingColdEmpty.cfg` checks eventual cold initialization with permanently empty endpoints under explicit weak fairness. `XdsEnvoyWarming.cfg` checks the safe behavior. The buggy configs intentionally fail when CDS ACK is treated as route-usable before EDS, an empty CLA is treated as usable, RDS is activated before its referenced cluster is usable, or LDS is activated before RDS exists.
...
`WarmTypeStarvation.tla` is the RF-002 temporal model: after a warm publication, a new route targets a permanently empty backend while an independent route/secret change is pending. Weak fairness forces repeated publication but never supplies endpoints. `WarmTypeStarvationCurrent.cfg` is expected to fail its progress property, reproducing the whole-type hold. `WarmTypeStarvationIsolated.cfg` specifies a proposed independent-component policy and passes; it assumes a valid dependency partition rather than computing one. See `../warm-isolation-obligation.md`.

`ColdMissingCdsStarvation.tla` is the RF-003 temporal model: a proxy with no cached snapshot has a candidate referencing a cluster absent from CDS and not recorded as errored. `ColdMissingCdsStarvationCurrent.cfg` (permanent absence) is expected to fail `EventuallyPublished`: the first publication is withheld forever while unrelated input keeps rebuilding. `ColdMissingCdsStarvationTransient.cfg` assumes the environment eventually supplies the cluster and passes. `ColdMissingCdsStarvationClassified.cfg` specifies a proposed policy that records a still-unresolved reference as errored after an abstract bound and passes. The invariant `NoSilentDanglingPublish` holds in all three: a published snapshot never carries an unresolved dynamic reference without an explicit classified outcome. The bound and its trigger are not chosen by the model.

`ReconnectWhileWarming.tla` is the RF-028 temporal model: a proxy whose CDS candidate is warming reconnects (controller restart) with an EDS request parked at a version equal to the re-derived one, and Envoy does not request CDS while warming, as measured on v1.39.1 over SotW. `ReconnectWhileWarmingCurrent.cfg` (no endpoint change, equal-version rule) is expected to fail `EventuallyRepaired`: a pending CDS-only repair never reaches the proxy while unrelated input keeps rebuilding. `ReconnectWhileWarmingEndpointChange.cfg` assumes the warming cluster's endpoints eventually change and passes. `ReconnectWhileWarmingUnconditionalFirstResponse.cfg` specifies the proposed policy of answering the first request on a new stream even at an equal version and passes. `ReconnectWhileWarmingEdsCacheFallback.cfg` models kgateway's deployed bootstrap, where `use_eds_cache_for_ads` completes the warming from the cached assignment when the unset (15 s) EDS fetch timeout expires, and passes; the Current configuration is therefore the shape with that timeout disabled. `NoCdsRequestWhileWarming` and `RepairNeedsRequest` hold in all three. Which repair to adopt is the RF-028 decision in the plan.

`KrtRecovery.tla` is the RF-005 / KRT-A1 temporal counterpart of the Lean recoverability theorem. A client left deferred-partial by a dropped fan-out converges only under an explicit fairness assumption: `KrtRecoveryCoherentInput.cfg` assumes KRT eventually delivers the missing dependency and passes; `KrtRecoveryDroppedFanout.cfg` assumes neither delivery nor a watchdog and is expected to fail `DeferredEventuallyActive`; `KrtRecoveryWatchdog.cfg` assumes a fair watchdog that rereads authoritative inputs and passes; `KrtRecoveryWatchdogStaleReplay.cfg` assumes a fair watchdog that replays the stuck cached derivation and is expected to fail. No watchdog exists in the deployed code, so the deployed profile is the first configuration: its progress claim rests entirely on KRT delivery.
...
- Envoy startup and make-before-break ordering: CDS ACK is distinct from route usability, EDS can be missing, empty, or ready, empty CLAs permit cold initialization, warm route flips require ready endpoints, and LDS must follow RDS for listener activation.
...
java -jar /path/to/tla2tools.jar -config XdsEnvoyWarmingEmptyCLAFlipBug.cfg XdsEnvoyWarming.tla
...
- An `XdsEnvoyWarming` `WarmRouteFlipHasReadyCLA` failure means the warm route-flip policy admitted a new backend with empty endpoints. Cold initialization with an empty CLA is valid.
