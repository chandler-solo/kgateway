...
// outcomes are memoized; transient errors such as exec failures or context
...
// text verbatim (empty on success); it is rehydrated as an error that satisfies
// errors.Is(err, ErrInvalidXDS) without re-wrapping, so the message renders
// exactly as the inner validator produced it.
...
func (c *cachingValidator) Validate(ctx context.Context, bootstrap *envoybootstrapv3.Bootstrap) error {
	...
	// which is stable within one process but should not be persisted or
	...
	// transiently, every waiter sees the transient error and nothing is cached.
