...
// content hash of the marshalled bootstrap config. Successful and ErrInvalidXDS
// outcomes are memoized; transient errors (e.g. exec failures, context
// cancellation) are not, so flaky underlying invocations don't get pinned in
// the cache. Concurrent misses on the same key are collapsed to one inner
// invocation via singleflight — during initial sync, independent collections
...
func cacheKeyFor(config string) string {
	sum := sha256.Sum256([]byte(config))
	return string(sum[:])
}

func (c *cachingValidator) Validate(ctx context.Context, config string) error {
	// The key is an IN-PROCESS cache key only: it hashes the protojson the
	// caller already produced, which is deterministic for a given binary but
	// deliberately unstable across builds (protojson varies whitespace via a
	// seed derived from a hash of the executable itself; see protobuf's
	// internal/detrand). It must never be persisted or compared across
	// versions; if that ever happens anyway, the failure mode is a cache miss,
	...
	res, sfErr, _ := c.sf.Do(key, func() (any, error) {
		innerErr := c.inner.Validate(ctx, config)
		if innerErr == nil {
