...
func BenchmarkValidator_DuplicateWorkload(b *testing.B) {
	...
	for _, tc := range []struct {
		...
		{"pooled", NewCaching(NewPooled(&fakeLatencyValidator{latency: benchLatency}, 0), 0)},
	} {
...
// Concurrent unique workload: the pooled pool's value is bounded parallelism
// across independently-initializing KRT collections during startup.
func BenchmarkValidator_ConcurrentUniqueWorkload(b *testing.B) {
	const distinct = 4096
	bootstraps := make([]*envoybootstrapv3.Bootstrap, distinct)
	for i := range distinct {
		bootstraps[i] = benchBootstrap(i)
	...
	for _, tc := range []struct {
		...
		{"binary", &fakeLatencyValidator{latency: benchLatency}},
		{"pooled", NewCaching(NewPooled(&fakeLatencyValidator{latency: benchLatency}, 0), distinct)},
	} {
		b.Run(tc.name, func(b *testing.B) {
			var n int64
			b.RunParallel(func(pb *testing.PB) {
				for pb.Next() {
					i := int(n) % distinct
					n++
					if err := tc.v.Validate(context.Background(), bootstraps[i]); err != nil {
						b.Fatal(err)
					}
				}
			})
