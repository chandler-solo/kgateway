...
func TestNew_PooledMode(t *testing.T) {
	v := New(apisettings.Settings{
		ValidatorMode:      apisettings.ValidatorPooled,
		ValidatorCacheSize: 16,
		ValidatorPoolSize:  4,
	})
	c, ok := v.(*cachingValidator)
	require.True(t, ok, "pooled mode should still be wrapped in *cachingValidator")
	d, ok := c.inner.(*pooledValidator)
	require.True(t, ok, "pooled mode should wrap *pooledValidator inside the cache")
	_, innerOK := d.inner.(*binaryValidator)
	assert.True(t, innerOK, "pooled mode should wrap *binaryValidator inside the pooled")
	assert.Equal(t, 4, cap(d.sem), "pool size from settings should be respected")
}

func TestNew_PooledModeZeroPoolFallsBackToDefault(t *testing.T) {
	v := New(apisettings.Settings{ValidatorMode: apisettings.ValidatorPooled})
	c := v.(*cachingValidator)
	d := c.inner.(*pooledValidator)
