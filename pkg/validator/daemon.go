...
// DefaultPoolSize is the default worker count for daemonValidator when no
...
// daemonValidator wraps an inner Validator with a bounded concurrency pool.
...
// NewDaemon wraps v with a bounded concurrency pool of the given size.
// If poolSize <= 0, DefaultPoolSize is used.
func NewDaemon(v Validator, poolSize int) Validator {
	if poolSize <= 0 {
...
func (d *daemonValidator) Validate(ctx context.Context, bootstrap *envoybootstrapv3.Bootstrap) error {
	select {
	case d.sem <- struct{}{}:
	case <-ctx.Done():
		return ctx.Err()
