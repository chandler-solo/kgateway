...
func TestGetTLSRouteForStatus(t *testing.T) {
	t.Run("prefers promoted v1 tlsroute when present", func(t *testing.T) {
		kubeClient := newFakeTLSRouteClient(t, tlsRouteV1Unstructured("route"))
		...
		require.Equal(t, schema.GroupVersionKind{Group: wellknown.GatewayGroup, Version: gwv1.GroupVersion.Version, Kind: wellknown.TLSRouteKind}, route.GetObjectKind().GroupVersionKind())
	})
	...
	t.Run("falls back to v1alpha2 tlsroute when newer route versions are absent", func(t *testing.T) {
...
func newFakeTLSRouteClient(t *testing.T, route ctrlclient.Object) ctrlclient.Client {
	...
	require.NoError(t, gwv1a2.Install(scheme))
...
func tlsRouteV1Unstructured(name string) *unstructured.Unstructured {
	route := &unstructured.Unstructured{}
	route.SetGroupVersionKind(schema.GroupVersionKind{Group: wellknown.GatewayGroup, Version: gwv1.GroupVersion.Version, Kind: wellknown.TLSRouteKind})
	route.SetName(name)
	route.SetNamespace("default")
	route.Object["spec"] = map[string]any{
		"parentRefs": []any{
