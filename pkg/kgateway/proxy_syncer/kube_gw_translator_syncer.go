...
func (s *ProxyTranslator) syncXds(
	...
	if snapWrap.deferred {
		// Per-cluster readiness resolution (spec:
		// devel/formal/lean/XdsSpec/PerClusterReadiness.lean). The snapshot
		// was built while some referenced cluster was not ready; decide per
		// cluster against the currently-published snapshot instead of
		// withholding everything:
		//
		//   - previously-published clusters that vanished from the build are
		//     carried forward with their CLAs (make-before-break, C5 pairs);
		//   - previously-referenced clusters whose endpoints scaled to zero
		//     publish their truth — the empty CLA — so Envoy stops routing
		//     to endpoints that no longer exist (C2);
		...
		//     flip goes out in a later snapshot once they are usable (C3 —
		//     which also gives the flip the reference-ahead shape: the CDS
		//     carrying a new cluster is published in an earlier snapshot than
...
// publishedRefsForTrace picks the referenced set matching what the composed
// snapshot actually publishes: the held routes' references when the flip is
// held, else the new routes' references are already in the wrapper trace.
func publishedRefsForTrace(snap *envoycache.Snapshot, publishedRefs map[string]struct{}) map[string]struct{} {
	// The composed snapshot either kept the new routes (truth publish) or the
	// previously-published ones (held flip); recompute from what it carries
	// so the trace reflects reality.
	routes := snap.Resources[envoycachetypes.Route]
	listeners := snap.Resources[envoycachetypes.Listener]
	if len(routes.Items) == 0 && len(listeners.Items) == 0 {
...
func resolveDeferredPerCluster(snapWrap XdsSnapWrapper, published envoycache.ResourceSnapshot) *envoycache.Snapshot {
	...
	*composed = *snapWrap.snap
	...
	// Carry forward cluster/CLA pairs (C5: a carried CLA always travels with
