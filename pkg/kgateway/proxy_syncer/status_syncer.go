...
func (s *StatusSyncer) syncRouteStatus(ctx context.Context, logger *slog.Logger, rm reports.ReportMap) {
		...
		return retry.Do(
			func() (rErr error) {
				...
				case *unstructured.Unstructured:
					converted := collections.ConvertUnstructuredTLSRouteToV1Alpha2(r)
					if converted != nil {
						for _, parentRef := range converted.Spec.ParentRefs {
							gatewayNames = append(gatewayNames, string(parentRef.Name))
						}
					}
				case *gwv1.GRPCRoute:
	...
	buildAndUpdateStatus := func(route client.Object, routeType string) (*gwv1.RouteStatus, error) {
		...
		case *unstructured.Unstructured:
			...
			status = rm.BuildRouteStatus(ctx, converted, s.controllerName)
			if status == nil || isRouteStatusEqual(&converted.Status.RouteStatus, status) {
				return nil, nil
			}
			rawStatus, err := collections.BuildUnstructuredTLSRouteStatus(gwv1a2.TLSRouteStatus{RouteStatus: *status})
			if err != nil {
				return nil, err
			}
			updated := r.DeepCopy()
			if err := unstructured.SetNestedField(updated.Object, rawStatus, "status"); err != nil {
				return nil, err
			}
			return status, s.mgr.GetClient().Status().Update(ctx, updated)
		case *gwv1.GRPCRoute:
			status = rm.BuildRouteStatus(ctx, r, s.controllerName)
			if status == nil || isRouteStatusEqual(&r.Status.RouteStatus, status) {
				return nil, nil
...
func getTLSRouteForStatus(ctx context.Context, kubeClient objectGetter, key client.ObjectKey) (client.Object, error) {
	...
	promotedTLSRoute.SetGroupVersionKind(collections.PromotedTLSRouteGVK)
