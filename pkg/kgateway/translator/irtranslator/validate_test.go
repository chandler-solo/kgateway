...
// countingValidator records calls and returns programmed responses. It returns
// failureFunc(i) for the i-th call (or nil if failureFunc is unset).
type countingValidator struct {
	calls       atomic.Int32
	failureFunc func(call int) error
}

func (c *countingValidator) Validate(_ context.Context, _ string) error {
	n := c.calls.Add(1)
...
func TestValidateRoute_StaticFailureIsRouteError(t *testing.T) {
	v := &countingValidator{}
	// "//" is in invalidPathSequences — caught by the in-process check.
...
func TestValidateRoute_StrictRouteActionFailure(t *testing.T) {
	...
	v := &countingValidator{
		failureFunc: func(call int) error {
			if call == 1 {
				return errors.New("cluster reference broken: " + validator.ErrInvalidXDS.Error())
			}
			return nil
		},
	}
